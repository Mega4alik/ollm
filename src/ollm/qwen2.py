# Qwen2.5-14B (Qwen2 Architecture / Llama-compatible)
...
from transformers.models.qwen2.modeling_qwen2 import Qwen2ForCausalLM, Qwen2MLP, Qwen2DecoderLayer, Qwen2Model, Qwen2Config, create_causal_mask, Cache
try:
...
class MyQwen2DecoderLayer(Qwen2DecoderLayer, loaderLayer):
...
class MyQwen2Model(Qwen2Model):
	def __init__(self, config):
		...
		for layer_idx in range(config.num_hidden_layers):
			self.layers.append(MyQwen2DecoderLayer(config, layer_idx))
	...
	def forward(
		...
		**kwargs: Any,
		...
		if stats: print("./Qwen2.forward.", datetime.now().strftime("%H:%M:%S"), stats.print_and_clean() if stats else "")
		#====================================
...
qwen2_modeling.Qwen2MLP = MyQwen2MLP
qwen2_modeling.Qwen2DecoderLayer = MyQwen2DecoderLayer
qwen2_modeling.Qwen2Model = MyQwen2Model
#===============================================
...
class MyQwen2ForCausalLM(Qwen2ForCausalLM, oForGeneration):
	def __init__(self, config):
		super().__init__(config)
