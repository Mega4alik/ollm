...
try:
    from transformers.models.qwen2.modeling_qwen2 import Qwen2MLP as Qwen3MLP
    from transformers.models.qwen2.modeling_qwen2 import Qwen2DecoderLayer as Qwen3DecoderLayer
    from transformers.models.qwen2.modeling_qwen2 import Qwen2Model as Qwen3Model
    from transformers.models.qwen2.modeling_qwen2 import Qwen2ForCausalLM as Qwen3ForCausalLM
    from transformers.models.qwen2.modeling_qwen2 import Qwen2Config as Qwen3Config
    from transformers.models.qwen2.modeling_qwen2 import create_causal_mask, Cache
    try:
...
class MyQwen3Model(Qwen3Model):
	...
	def forward(
		...
		**kwargs: Any,
        ...
        # Qwen2/3 doesn't have parent_lm_head attached to model usually, but our pattern might add it.
        # Check if it exists before accessing.
		if hasattr(self, "parent_lm_head"): self.parent_lm_head.cpu()
		...
		if hasattr(self, "parent_lm_head"): self.parent_lm_head.to(hidden_states.device)
...
class MyQwen3ForCausalLM(Qwen3ForCausalLM, oForGeneration):
	def __init__(self, config):
		super().__init__(config)
